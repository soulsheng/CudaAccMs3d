...
int main(int argc, char** argv)
{
    ...
    // create the kernel
    ckKernel = clCreateKernel(cpProgram, "updateVectorByMatrix4", &ciErrNum);
    shrCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
#if 1
	
	PROBLEM_SIZE  = MEGA_SIZE * PROBLEM_SCALE[iClass] ;
	mmv.initialize( PROBLEM_SIZE, JOINT_SIZE );

	mmv.SetupKernelVBO( cxGPUContext, cdDevices[uiDeviceUsed], ckKernel, cqCommandQueue );
