...
bool CMatrixMulVector::ExecuteKernelVBO()
{
	...
	double dTime = shrDeltaT(0); 

	insertTimer("11.Acquire GL", dTime);

	cl_event g_perf_event = NULL;
	cl_int  eRet = clEnqueueNDRangeKernel(_cmd_queue, _kernel, 2, NULL, globalWorkSize, localWorkSize, 0, NULL, &g_perf_event);
...
void CMatrixMulVector::printfTimer()
{
